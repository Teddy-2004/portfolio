    ...
    <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 1 }}>
      {shapes.map((s, i) => (
        <motion.div key={i}
          ...
          {s.type === "hex" && <svg width={s.size} height={s.size} viewBox="0 0 100 100"><polygon points="50,5 90,25 90,75 50,95 10,75 10,25" fill="none" stroke={s.color} strokeWidth="1.5" opacity="0.6" /></svg>}
          {s.type === "tri" && <svg width={s.size} height={s.size} viewBox="0 0 100 100"><polygon points="50,10 90,80 10,80" fill="none" stroke={s.color} strokeWidth="1.5" opacity="0.6" /></svg>}
          {s.type === "sq"  && <svg width={s.size} height={s.size} viewBox="0 0 100 100"><rect x="10" y="10" width="80" height="80" fill="none" stroke={s.color} strokeWidth="1.5" opacity="0.6" transform="rotate(45 50 50)" /></svg>}
        </motion.div>
      ))}
    </div>
  );
...
const GlowBtn: FC<GlowBtnProps> = ({ children, variant = "primary", onClick, href, download }) => {
  const base: CSSProperties = { display: "inline-flex", alignItems: "center", gap: 8, padding: "14px 28px", borderRadius: 40, fontSize: 14, fontFamily: "'Syne',sans-serif", fontWeight: 600, cursor: "pointer", letterSpacing: 0.5, border: "none", textDecoration: "none" };
...
const Hero: FC = () => {
  ...
  return (
    <motion.div style={{ y, opacity, position: "relative", zIndex: 2, minHeight: "100vh", display: "flex", alignItems: "center", paddingTop: 100 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 32 }}>
            {stats.map(s => (
              <motion.div key={s.l} whileHover={{ scale: 1.04 }}
                style={{ padding: "24px", borderRadius: 16, border: "1px solid rgba(14,165,233,0.2)", background: "rgba(14,165,233,0.04)", textAlign: "center" }}>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 40 }}>
        {Object.keys(TECH).map(cat => (
          <motion.button key={cat} data-hover onClick={() => setActive(cat)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
            style={{ padding: "8px 20px", borderRadius: 20, border: `1px solid ${active === cat ? C.blue : "rgba(255,255,255,0.12)"}`,
        ...
        <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}
          ...
          {(TECH[active] ?? []).map((tech: string, i: number) => (
            <motion.div key={tech} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }}
              ...
              style={{ padding: "10px 20px", borderRadius: 40, background: "rgba(14,165,233,0.07)", border: "1px solid rgba(14,165,233,0.25)", color: "#e2e8f0", fontSize: 13, fontWeight: 500 }}>
              {tech}
            </motion.div>
          ))}
        </motion.div>
...
const ProjectCard: FC<ProjectCardProps> = ({ p, i }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.6 }}
      ...
      style={{ borderRadius: 20, border: `1px solid ${hov ? p.color : "rgba(255,255,255,0.08)"}`,
        background: hov ? `${p.color}0d` : "rgba(255,255,255,0.02)",
        padding: 28, position: "relative", overflow: "hidden", transition: "all 0.4s",
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {p.tech.map(t => (
          <span key={t} style={{ fontSize: 10, padding: "3px 10px", borderRadius: 20, background: `${p.color}15`, border: `1px solid ${p.color}30`, color: p.color, fontWeight: 600 }}>{t}</span>
        ))}
      </div>
    </motion.div>
  );
...
const Contact: FC = () => {
  ...
  const inputStyle: CSSProperties = { width: "100%", padding: "14px 20px", borderRadius: 12, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(14,165,233,0.2)", color: "#fff", fontSize: 14, fontFamily: "'Syne',sans-serif", outline: "none" };
  return (
    <Section id="contact">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start" }}>
        <div>
          ...
          {(["✉  tedla@example.com", "🔗  linkedin.com/in/yourusername", "⌨  github.com/yourusername"]).map(line => (
            <div key={line} style={{ color: "rgba(255,255,255,0.6)", fontSize: 14, marginBottom: 16 }}>{line}</div>
          ))}
        </div>
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <motion.button type="submit" data-hover whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            style={{ padding: "14px 32px", borderRadius: 40, background: `linear-gradient(135deg, ${C.violet}, ${C.blue})`, border: "none", color: "#fff", fontSize: 14, fontFamily: "'Syne',sans-serif", fontWeight: 600, cursor: "pointer" }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
      {CERTS.map((c, i) => (
        <motion.div key={c.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
          ...
          style={{ padding: "20px 24px", borderRadius: 16, border: "1px solid rgba(14,165,233,0.15)", background: "rgba(14,165,233,0.03)" }}>
...
const AIStats: FC = () => {
  const stats: StatItem[] = [
    ...
    { val: 98, suffix: "%",  label: "Best Model Accuracy"  },
